Set_local_parameters_custom_removal_fast2 <- function(pts,HL,cons,chem,chem_ii,UseCpp=TRUE){

  # if C++ is not used
  if(!UseCpp) SimpleTreat4_0_cpp = SimpleTreat4_0
  ...
  if (nrow(HL)!=0) HL$T_AIR <- HL$T_AIR+273.15           #air temperature in K
  ...
  pts$pH          <- 7.4         #pH of local surface water, proposed default pH of 7.4 (Meybeck et al 1995)
  ...
  if (nrow(HL)!=0) {
    ...
    HL$pH          <- 7.4                              #pH of local surface water, proposed default pH of 7
  ...
  if(useSimpleTreat4_0){
    for (j in idx_WWTPs) {
      if (pts$Pt_type[j]=="WWTP") {
        ...
        pts$f_rem_WWTP[j] <-  ifelse(pts$f_STP[j]==0,0,SimpleTreat4_0_cpp(chem$class[chem_idx],chem$MW[chem_idx],chem$Pv[chem_idx],chem$S[chem_idx],chem$pKa[chem_idx],
                                             chem$Kp_ps[chem_idx],chem$Kp_as[chem_idx],chem$k_bio_wwtp[chem_idx],pts$T_AIR[j],
